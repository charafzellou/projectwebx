<div>
    <div id="menu-header-logo" class="col-lg-2">
        <img src="img/logonoback.png" width="180px"/>
    </div>
    
    <div id="menu-header-top">
        <div href="#" class="col-lg-7" style ="background: red;">
            Météo
        </div>
        <div href="#" class="dropdown col-lg-3" style ="background: orange;">Mon compte
        	<ul>
                <li><a id="connection-item" href="#">Se connecter</a></li>
				<li><a id="connection-item" href="#">Créer un compte</a></li>
			</ul>
        </div>
    </div>
</div>        

<nav id="menu-header-bottom" class="col-lg-12">
    <ul id="menu-primary-bottom" class="menu row">
        <li class="menu-item menu-item-type-post_type menu-item-object-page first col-lg-3">
            <a href="#">Accueil</a>
        </li>
        <li class="menu-item menu-item-type-post_type menu-item-object-page col-lg-3">
            <a href="#">Services</a>
        </li>
        <li class="menu-item menu-item-type-post_type menu-item-object-page col-lg-3">
            <a href="#">Aéroclub</a>
        </li>
        <li class="menu-item menu-item-type-post_type menu-item-object-page last  dropdown col-lg-3">
            <a href="#">A propos</a>
        </li>
    </ul>
</nav>